...
/// JinaBertEncoder stacks 12 JinaBertLayer instances
///
/// Holds the ALiBi slopes permanently (48 bytes) and computes the full bias matrix
/// once per forward call, sharing it across all 12 layers.
/// This avoid recomputing the distance matrix 12 times per inference.
...
impl JinaBertEncoder {
    fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        // Guard: this implementation supports only ALiBi positional encoding.
        // Learned positional embeddings would require a different code path and
        // would limit sequence length extrapolation to the trained maximum
        if cfg.position_embedding_type != PositionEmbeddingType::Alibi {
            candle_core::bail!("only alibi position embedding is supported")
        }

        let layers = (0..cfg.num_hidden_layers)
            .map(|idx| JinaBertLayer::new(vb.pp(format!("layer.{idx}")), cfg))
            .collect::<Result<Vec<_>>>()?;

        // move slopes to the model's device (CPU in practice for this provider)
        let alibi_slopes = build_alibi_slopes(cfg)?.to_device(vb.device())?;
        Ok(Self {
            alibi_slopes,
            layers,
...
impl Module for JinaBertEncoder {
    /// xs : [batch, seq_len, hidden_size=768]
    /// out: [batch, seq_len, hidden_size=768] -> after 12 layers of attention + FFN
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let _enter = self.span.enter();
        let seq_len = xs.dim(1)?;

        // Compute Alibi bias for the actual sequence length (not the 8192 token maximum )
        // this single [1,12,seq_len, seq_len] Tensor is reused by all 12 layers
        let bias = compute_alibi_bias(&self.alibi_slopes, seq_len, xs.device())?;

        let mut xs = xs.clone();
        for layer in &self.layers {
            xs = layer.forward(&xs, &bias)?;
        }
        Ok(xs)
    }
}

/// JinaBertModel: Public entry point, drop-in replacement for candle's BertModel
///
/// Exposes the same constructor signature as candle's BertModel:
///     JinaBertModel::new(vb: VarBuilder, cfg: &Config) -> Result<Self>
///     impl Module: forward(input_ids: &Tensor) -> Result<Tensor>
///
/// mod.rs creates this model inside CandleProvider::new() and stores it as
/// Arc<JinaBertModel>. Mean-pooling over seq_len and normalization to unit norm
/// are performed by the caller ( embed_single in mod.rs), not here
///
...
pub struct JinaBertModel {
    ...
    /// exposed so mod.rs can place input tensors on the same device
    pub device: Device,
    span: tracing::Span,
}
...
impl JinaBertModel {
    pub fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        let encoder = JinaBertEncoder::new(vb.pp("encoder"), cfg)?;
        Ok(Self {
            embeddings,
            encoder,
...
impl Module for JinaBertModel {
    /// input_ids: [batch, seq_len]      -> integer token IDs from the tokenizer
    /// output:    [batch, seq_len, 768] -> contextualized token vectors
    /// (caller mean-pools -> [batch, 768] and normalices to unir norm)
    fn forward(&self, input_ids: &Tensor) -> Result<Tensor> {
        let _enter = self.span.enter();
        // token IDs -> initial embedding vectors: [batch, seq] -> [batch, seq, 768]
        let embeddings = self.embeddings.forward(input_ids)?;
        // 12 layers of attention + FFN: [batch, seq, 768] -> [batch, seq, 768]
